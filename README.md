...
- 点線（自信のない読み。画面だけ、設定で印刷にも。D70）: 漢字 1 字だけの語、漢字だけのトークンが 2 つ以上続いて割れた語、漢字どうしが語の切れ目で接する解析器の語（無暗に → 無/暗に。両側）、読み分けのある語（追加辞書で読みが 2 つ以上の語と、`constants.js` の `ambiguousReadings` 60 語。直す画面にほかの読みを出す）、送り仮名の重なりを外した 2 字以上の語（身近か）、「1日」
- 送り仮名の重なり: IPADIC の見出しに送り仮名が無い語（俄 = にわか）や古い送り仮名（短かい・疑ぐる）では、読みの終わりが本文の続きの仮名と重なる。形容動詞の語幹・形容詞・動詞・連体詞に限って重なりを読みから外す（俄か → 俄《にわ》か）
- 読み分けのある語の表: `tools/ambiguous-candidates.txt`（自分で選んだ語と見出し）を `python3 tools/verify-ambiguous.py` がデジタル大辞泉の見出しで確かめ、`tools/ambiguous-verified.tsv`（出典 URL つき）に。見出しで確かめられなかった読み（上り → あがり、下り → おり・さがり、十分 → じっぷん、弾く → ひく）は入れていない。表と tsv が同じことは `tests/data.test.js` が確かめる
...
- オフライン: Service Worker（キャッシュ `furigana-v4`、辞書は別のキャッシュ `furigana-dict-lindera-2.0.0`）。辞書は解析するときに初めて取る
...
|---|---|---|---|---|---|---|
| lindera 2.0.0 | あり | **96.9%**（826/852） | 81.9%（484/591） | 41.0% | 68 | 92 |
| lindera 2.0.0 | なし | 96.8%（825/852） | 81.0%（478/590） | 40.9% | 68 | 93 |
| （読み分けの表・送り仮名の重なり・接する漢字の印を入れる前） | | | | | | |
...
- 2026-09-24 の直し（オーナーが青空文庫の文を貼って見つけた 俄か・無暗・行った・開けて）のあと、点線なしは 96.9%（826/852 = 96.95%）で合格基準（D33・企画書 7 章）の 96.9% にちょうど届いた。数字に合わせて規則を選んだのではなく、直しの結果を測ったもの。振ったルビ全体（比べられないものも含む）では点線が 43.1% → 45.2% に増えた
- 点線なしで残る誤りは、送り仮名つきの 1 字の語（上り → のぼ／あが、廻る）、お前（まい）・その間（あいだ）など。上り の あがり はデジタル大辞泉の【上り】の見出しに無いので表に入れていない
...
- 大人向けを含む 12 作品では点線なし 90.7%（2,657/2,931）。直す前は 89.9%（2,722/3,028）
